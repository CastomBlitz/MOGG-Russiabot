...
def client_order_kb(order):
    ...
    if status == STATUS_ACCEPTED:
        return finish_deal_kb(num)
    if status == STATUS_DEAL:
        return deal_choice_kb(num)
    return contact_kb(store.get())
...
async def handle_deal_action(update, context, rest):
    ...
    if order is None or action not in ("finish", "ok", "fail"):
        ...
        return
    if user.id not in (OWNER_ID, order.get("user_id")):
    ...
    if action == "finish":
        ...
        other = order["user_id"] if user.id == OWNER_ID else OWNER_ID
        await send_safe(context.bot, other, text, deal_choice_kb(number))
        return
    ...
    if success:
        ...
        await send_safe(context.bot, OWNER_ID, f"🎉 Сделка #{number} состоялась.")
    else:
        text = (
...
async def handle_review_text(update, context, text):
    ...
    await send_safe(context.bot, OWNER_ID, f"⭐ Новый отзыв по сделке #{number}: {rating}/5\n{esc(text.strip())}")
