...
OWNER_ID = 5139892680
# Дополнительные администраторы. Их Telegram ID хранится только внутри бота.
ADMIN_IDS = {OWNER_ID, 5654866393}
...
STATUS_DEAL = "Сделка идёт"
STATUS_SUCCESS = "Сделка состоялась"
STATUS_FAILED = "Сделка сорвалась"
STATUS_ICONS = {STATUS_NEW: "🆕", STATUS_ACCEPTED: "✅", STATUS_REJECTED: "❌", STATUS_DEAL: "🤝", STATUS_SUCCESS: "🎉", STATUS_FAILED: "❌"}
...
def is_owner(update):
    # Владелец сохраняет доступ к управлению скидками/отзывами.
    user = update.effective_user
...
def contact_kb(data, back_target="m:main"):
    rows = [[btn("💬 Связаться через бота", "m:support")]]
...
def order_kb(order):
    ...
    rows = [[btn("💬 Чат по сделке", f"chat:open:{num}")]]
    if status == STATUS_NEW:
        rows.append([btn("✅ Принять", f"ord:a:{num}"), btn("❌ Отклонить", f"ord:r:{num}")])
    elif status == STATUS_ACCEPTED:
        rows.append([btn("🤝 Сделка завершена", f"deal:finish:{num}")])
    elif status == STATUS_DEAL:
        rows.append([btn("🎉 Сделка состоялась", f"deal:ok:{num}"), btn("❌ Сделка сорвалась", f"deal:fail:{num}")])
    return InlineKeyboardMarkup(rows)


def client_order_kb(order):
    num = order["number"]
    status = order.get("status")
    rows = [[btn("💬 Чат по сделке", f"chat:open:{num}")]]
...
def deal_choice_kb(number):
    return InlineKeyboardMarkup([
        [btn("🎉 Сделка состоялась", f"deal:ok:{number}")],
        [btn("❌ Сделка сорвалась", f"deal:fail:{number}")],
    ])


def finish_deal_kb(number):
    return InlineKeyboardMarkup([[btn("🤝 Сделка завершена", f"deal:finish:{number}")]])


def review_stars_kb(number):
    return InlineKeyboardMarkup([
        [btn("⭐ 1", f"review:rate:{number}:1"), btn("⭐⭐ 2", f"review:rate:{number}:2")],
        [btn("⭐⭐⭐ 3", f"review:rate:{number}:3"), btn("⭐⭐⭐⭐ 4", f"review:rate:{number}:4")],
        [btn("⭐⭐⭐⭐⭐ 5", f"review:rate:{number}:5")],
    ])
...
def format_order_message(o):
    ...
    buyer = f"@{esc(o['username'])}" if o.get("username") else "username не указан"
    return (
        f"{header}\n\n"
        f"Номер: <b>#{o['number']}</b>\n\n"
        f"👤 ЮЗ покупателя: <b>{buyer}</b>\n"
...
def deal_chat_intro(order, for_admin=False):
    buyer = f"@{esc(order.get('username'))}" if order.get('username') else "покупатель без username"
    if for_admin:
        return (f"💬 <b>Анонимный чат сделки #{order['number']}</b>\n\n"
                f"Вы общаетесь с покупателем <b>{buyer}</b>.\n"
                "Ваш личный Telegram-профиль покупателю не показывается.\n\n"
                "Отправьте сообщение текстом — бот передаст его покупателю.")
    return (f"💬 <b>Анонимный чат сделки #{order['number']}</b>\n\n"
            "Вы общаетесь с администратором через бота.\n"
            "Личный Telegram-профиль администратора вам не показывается.\n\n"
            "Отправьте сообщение текстом — бот передаст его администратору.")
...
async def choose_payment(update, context, key):
    ...
    for admin_id in admin_ids():
        sent = await send_safe(context.bot, admin_id, format_order_message(order), order_kb(order))
        ok = ok or sent
    if ok:
...
async def handle_order_action(update, context, rest):
    ...
    await send_safe(context.bot, order["user_id"], text, client_order_kb(order) if action == "a" else contact_kb(store.get()))


async def find_order(number):
    data = store.get()
    return data, next((o for o in data["orders"] if o.get("number") == number), None)


async def notify_deal_participants(bot, order, text, kb=None, exclude_id=None):
    ids = set(ADMIN_IDS) | {order["user_id"]}
    if exclude_id in ids:
        ids.remove(exclude_id)
    for uid in ids:
        await send_safe(bot, uid, text, kb)


async def handle_deal_action(update, context, rest):
    query = update.callback_query
    user = update.effective_user
    action, _, num_text = rest.partition(":")
    try:
    ...
    data, order = await find_order(number)
    if order is None or action not in ("finish", "ok", "fail"):
        await safe_answer(query, "Сделка не найдена", alert=True)
        return
    if user.id not in (set(ADMIN_IDS) | {order.get("user_id")}):
        await safe_answer(query, "⛔ Вы не участник этой сделки", alert=True)
        return

    if action == "finish":
        if order.get("status") != STATUS_ACCEPTED:
            await safe_answer(query, "Сейчас нельзя завершить этот этап сделки", alert=True)
            return
        order["status"] = STATUS_DEAL
        order["completion_requested_by"] = user.id
        order["updated_at"] = datetime.now(MSK).isoformat(timespec="seconds")
        store.save()
        await safe_answer(query, "Вы отметили сделку как завершённую")
        text = (
            f"🤝 <b>Заявка #{number}: сделка готова к завершению</b>\n\n"
            "Выберите итог сделки:"
        )
        await render(update, context, text, deal_choice_kb(number))
        if user.id in ADMIN_IDS:
            await send_safe(context.bot, order["user_id"], text, deal_choice_kb(number))
        else:
            for admin_id in ADMIN_IDS:
                await send_safe(context.bot, admin_id, text, deal_choice_kb(number))
        return

    if order.get("status") != STATUS_DEAL:
        await safe_answer(query, "Сначала нажмите «Сделка завершена»", alert=True)
        return

    success = action == "ok"
    order["status"] = STATUS_SUCCESS if success else STATUS_FAILED
    order["updated_at"] = datetime.now(MSK).isoformat(timespec="seconds")
    order["closed_by"] = user.id
    store.save()
    await safe_answer(query, "Сделка успешно закрыта" if success else "Сделка отмечена как сорвавшаяся")

    if success:
        text = (
            f"🎉 <b>Сделка #{number} успешно завершена!</b>\n\n"
            f"Админка: {order['level']} уровень\n"
            f"Игровой ник: <code>{esc(order['nick'])}</code>\n\n"
            "Спасибо за покупку! Пожалуйста, оцените сделку от 1 до 5 звёзд:"
        )
        await send_safe(context.bot, order["user_id"], text, review_stars_kb(number))
        for admin_id in ADMIN_IDS:
            await send_safe(context.bot, admin_id, f"🎉 Сделка #{number} состоялась.")
    else:
        text = (
            f"❌ <b>Сделка #{number} отмечена как сорвавшаяся.</b>\n\n"
            "Отзыв оставлять не нужно. Если хотите разобраться с ситуацией, свяжитесь с администратором."
        )
        await notify_deal_participants(context.bot, order, text)

    await render(update, context, format_order_message(order), order_kb(order))


async def handle_review_action(update, context, rest):
    query = update.callback_query
    parts = rest.split(":")
    if len(parts) != 3 or parts[0] != "rate":
        await safe_answer(query, "Некорректный отзыв", alert=True)
        return
    try:
        number, rating = int(parts[1]), int(parts[2])
    except ValueError:
        await safe_answer(query, "Некорректная оценка", alert=True)
        return
    if rating not in range(1, 6):
        await safe_answer(query, "Оценка должна быть от 1 до 5", alert=True)
        return
    data, order = await find_order(number)
    if order is None or order.get("user_id") != update.effective_user.id or order.get("status") != STATUS_SUCCESS:
        await safe_answer(query, "Этот отзыв недоступен", alert=True)
        return
    if order.get("review_id") is not None:
        await safe_answer(query, "Вы уже оставили отзыв", alert=True)
        return
    context.user_data.clear()
    context.user_data["state"] = "review_text"
    context.user_data["review_order"] = number
    context.user_data["review_rating"] = rating
    await safe_answer(query, f"Оценка: {rating}/5")
    await render(update, context, f"⭐ Вы поставили <b>{rating}/5</b>.\n\nТеперь напишите текст отзыва (до {MAX_REVIEW_LEN} символов):", InlineKeyboardMarkup([back_row("m:main")]))


async def handle_review_text(update, context, text):
    ud = context.user_data
    try:
        number = int(ud.get("review_order"))
        rating = int(ud.get("review_rating"))
    except (TypeError, ValueError):
        ud.clear()
        await show_main(update, context)
        return
    if not text.strip():
        await render(update, context, "⚠️ Отзыв не может быть пустым. Напишите текст ещё раз.", InlineKeyboardMarkup([back_row("m:main")]))
        return
    if len(text) > MAX_REVIEW_LEN:
        await render(update, context, f"⚠️ Отзыв слишком длинный. Максимум {MAX_REVIEW_LEN} символов.", InlineKeyboardMarkup([back_row("m:main")]))
        return
    data, order = await find_order(number)
    if order is None or order.get("user_id") != update.effective_user.id or order.get("status") != STATUS_SUCCESS or order.get("review_id") is not None:
        ud.clear()
        await render(update, context, "⚠️ Этот отзыв уже нельзя изменить.", main_menu_kb())
        return
    review = {
        "author": order.get("nick") or update.effective_user.full_name or "Покупатель",
        "rating": rating,
        "text": text.strip(),
    ...
    order["review_id"] = len(data["reviews"])
    order["updated_at"] = datetime.now(MSK).isoformat(timespec="seconds")
    store.save()
    ud.clear()
    await render(update, context, "✅ <b>Спасибо за отзыв!</b>\n\nВаш отзыв сохранён в разделе «Отзывы». ⭐", main_menu_kb())
    for admin_id in ADMIN_IDS:
        await send_safe(context.bot, admin_id, f"⭐ Новый отзыв по сделке #{number}: {rating}/5\n{esc(text.strip())}")
...
async def cmd_orders(update, context):
    ...
    for o in reversed(orders[-15:]):
        who = f"@{esc(o['username'])}" if o.get("username") else "username не указан"
        lines.append(
...
async def discount_watchdog(context):
    ...
    if d.get("active") and d.get("expires_at", 0) <= time.time():
        ...
        for admin_id in ADMIN_IDS:
            await send_safe(context.bot, admin_id, "⏰ Скидка закончилась и отключена автоматически. Цены вернулись к обычным.")


# ============================================================================
#  АНОНИМНЫЙ ЧАТ СДЕЛКИ
# ============================================================================

async def handle_chat_callback(update, context, rest):
    query = update.callback_query
    action, _, num_text = rest.partition(":")
    try:
        number = int(num_text)
    except ValueError:
        await safe_answer(query, "Некорректная сделка", alert=True)
        return
    data, order = await find_order(number)
    if order is None:
        await safe_answer(query, "Сделка не найдена", alert=True)
        return
    if order.get("status") not in (STATUS_ACCEPTED, STATUS_DEAL):
        await safe_answer(query, "Чат откроется после принятия заявки", alert=True)
        return
    user = update.effective_user
    if user.id not in (set(ADMIN_IDS) | {order.get("user_id")}):
        await safe_answer(query, "⛔ Вы не участник этой сделки", alert=True)
    ...
    if action == "open":
        context.user_data.clear()
        context.user_data["state"] = "deal_chat"
        context.user_data["chat_order"] = number
        await safe_answer(query)
        await render(update, context, deal_chat_intro(order, user.id in ADMIN_IDS), InlineKeyboardMarkup([back_row("m:main")]))
        return
    await safe_answer(query, "Неизвестное действие", alert=True)


async def handle_deal_chat_text(update, context, text):
    ud = context.user_data
    try:
        number = int(ud.get("chat_order"))
    except (TypeError, ValueError):
        ud.clear()
        await show_main(update, context)
        return
    data, order = await find_order(number)
    user = update.effective_user
    if order is None or user.id not in (set(ADMIN_IDS) | {order.get("user_id")}):
        ud.clear()
        await show_main(update, context)
        return
    text = text.strip()
    if not text:
        return
    if len(text) > 1500:
        await render(update, context, "⚠️ Сообщение слишком длинное. Максимум 1500 символов.", InlineKeyboardMarkup([back_row("m:main")]))
        return
    if user.id in ADMIN_IDS:
        # Для покупателя администратор всегда отображается как единый бот-аккаунт.
        outgoing = (f"💬 <b>Сообщение по сделке #{number}</b>\n\n"
                    f"👤 Администратор: <b>MOGG RUSSIA</b>\n"
                    f"{esc(text)}")
        await send_safe(context.bot, order["user_id"], outgoing, client_order_kb(order))
    else:
        buyer = f"@{esc(order.get('username'))}" if order.get('username') else "username не указан"
        outgoing = (f"💬 <b>Сообщение по сделке #{number}</b>\n\n"
                    f"👤 ЮЗ покупателя: <b>{buyer}</b>\n"
                    f"{esc(text)}")
        for admin_id in ADMIN_IDS:
            await send_safe(context.bot, admin_id, outgoing, order_kb(order))
    await render(update, context, "✅ Сообщение отправлено через анонимный чат.", deal_chat_intro(order, user.id in ADMIN_IDS))
...
async def on_callback(update, context):
    ...
    if kind == "d":
        ...
        return
    if kind == "deal":
        await handle_deal_action(update, context, rest)
        return
    if kind == "review":
        await handle_review_action(update, context, rest)
        return
    if kind == "chat":
        await handle_chat_callback(update, context, rest)
        return
...
async def on_text(update, context):
    ...
    if state == "deal_chat":
        await handle_deal_chat_text(update, context, text)
    elif state == "nick":
...
async def on_non_text(update, context):
    ...
    if state == "deal_chat":
        await render(update, context, "⚠️ В анонимном чате отправляйте сообщения обычным текстом.", InlineKeyboardMarkup([back_row("m:main")]))
        return
    if state == "nick":
        await render(update, context, "⚠️ Пожалуйста, отправьте ник обычным текстом.",
                     InlineKeyboardMarkup([back_row("m:buy")]))
    elif state == "review_text":
        await render(update, context, "⚠️ Пожалуйста, отправьте отзыв обычным текстом.", InlineKeyboardMarkup([back_row("m:main")]))
